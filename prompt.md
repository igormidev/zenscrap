Context:
Zenscrap is a saas where the user talks to an AI that generates web scrappers that can be used in scrapping bee. 
...
The reason for this is because I think that instead of downloading the html/screenshot of the page and giving it to the ai is probabbly not the best path - it will not generate the best quality output. I first created a package called gemini_cli_sdk in @gemini_cli_sdk/ (that you can edit if needed) and then I am currently building that new logic in @web_scrapper_generator/ 
...
This mcp will basicly have only 1 functionality: Give the hability to the api to test a extract rule that it will create.
So it should receive a the following fields:
...
This mcp MUST be done in dart. Feel free to use any package like "https://pub.dev/packages/dart_mcp" to make implementation more easy (of use none packages at all if you think it is not needed).
