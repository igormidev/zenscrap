...
class ExtractFullDataByRule {
  ...
  T when<T>({
    required T Function(Map<String, dynamic> result, String html, Uint8List screenshot) withData,
    required T Function(String errorMessage) error,
  }) {
    if (hasError) {
      return error(errorMessage!);
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (response.data is String) {
            try {
              resultData = jsonDecode(response.data as String) as Map<String, dynamic>;
            } catch (e) {
        ...
        try {
          ...
          if (response.data is String) {
            try {
              errorResponse = jsonDecode(response.data as String) as Map<String, dynamic>;
            } catch (_) {
              return ExtractDataByRule.error(
                errorMessage: 'ScrapingBee API error: ${response.data} (Status: ${response.statusCode})',
              );
            }
          } else {
          ...
          return ExtractDataByRule.error(
            errorMessage: 'ScrapingBee API error (Status: ${response.statusCode})',
          );
        }
      }
      ...
      if (e.response != null) {
        try {
          ...
          if (e.response!.data is String) {
            try {
              errorDetails = jsonDecode(e.response!.data as String) as Map<String, dynamic>;
            } catch (_) {
              return ExtractDataByRule.error(
                errorMessage: 'ScrapingBee API error: ${e.response!.data} (Status: ${e.response!.statusCode})',
              );
            }
          } else {
          ...
          errorMessage = 'ScrapingBee API error (Status: ${e.response!.statusCode})';
        }
      } else if (e.type == DioExceptionType.connectionTimeout) {
        errorMessage = 'Connection timeout - the target site may be slow or unresponsive';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ExtractFullDataByRule.withData(
          ...
          html: response.data?['html'] ?? body, // Use 'html' field if available, otherwise use body
