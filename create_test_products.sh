...
BASIC_PRODUCT=$(stripe products create \
  --name="BASIC Plan" \
  --description="For side-projects - 250,000 API credits, 10 concurrent requests, 3 active endpoints" \
...
PRO_PRODUCT=$(stripe products create \
  --name="PRO Plan" \
  --description="For startups - 1,000,000 API credits, 30 concurrent requests, 10 active endpoints, access to best AI model" \
  --api-key sk_test_51RK4KXJRaWX4CIXemSfOIlUydnVoGXCkTmhN5Kx47FQNL9QpygmAhciuBdsXVIce0Z4XyJOnNUaiSUlfN8OvHZgL00xYLltPyQ | jq -r '.id')
...
ULTRA_PRODUCT=$(stripe products create \
  --name="ULTRA Plan" \
  --description="Enterprise usage - 4,000,000 API credits, 100 concurrent requests, 100 active endpoints, best AI model, priority support, marketplace features" \
  --api-key sk_test_51RK4KXJRaWX4CIXemSfOIlUydnVoGXCkTmhN5Kx47FQNL9QpygmAhciuBdsXVIce0Z4XyJOnNUaiSUlfN8OvHZgL00xYLltPyQ | jq -r '.id')
