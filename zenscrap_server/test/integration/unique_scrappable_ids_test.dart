  ...
  withServerpod('Given unique scrappable IDs with duration query', (
    ...
    test(
      ...
      () async {
        ...
        final result = await session.db.unsafeQuery(r'''
    ...
    test(
      'when called with pagination, should correctly limit and offset',
      () async {
        final session = sessionBuilder.build();
        ...
        final firstPage = await session.db.unsafeQuery(r'''
        SELECT DISTINCT "scrappableId"
        FROM "scrappable_analytics"
        WHERE "duration" IS NOT NULL
          AND "requestStatus" = 'success'
        ...
        final secondPage = await session.db.unsafeQuery(r'''
        SELECT DISTINCT "scrappableId"
        FROM "scrappable_analytics"
        WHERE "duration" IS NOT NULL
          AND "requestStatus" = 'success'
        ...
        final firstPageIds = firstPage.map((row) => row[0] as int).toList();
        final secondPageIds = secondPage.map((row) => row[0] as int).toList();
        ...
        expect(
          firstPageIds.toSet().intersection(secondPageIds.toSet()),
          isEmpty,
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        final sqlResult = await session.db.unsafeQuery(r'''
        SELECT DISTINCT "scrappableId"
        FROM "scrappable_analytics"
        WHERE "duration" IS NOT NULL
          AND "requestStatus" = 'success'
