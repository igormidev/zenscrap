...
class StripeWebhookRoute extends Route {
  ...
  /// Executor with concurrency of 1 ensures webhooks are processed sequentially.
  /// This prevents race conditions between checkout.session.completed and
  /// customer.subscription.created events that update the same account.
  static final _webhookExecutor = Executor(concurrency: 1);

  /// In-memory set of processed event IDs to prevent duplicate processing.
  /// Stripe may retry webhooks, and we must be idempotent.
  /// Events are cleared after 1 hour to prevent memory leaks.
  static final _processedEvents = <String, DateTime>{};
  static const _eventRetentionDuration = Duration(hours: 1);

  /// Check if an event has already been processed
  static bool _isEventProcessed(String eventId) {
    _cleanupOldEvents();
    return _processedEvents.containsKey(eventId);
  }

  /// Mark an event as processed
  static void _markEventProcessed(String eventId) {
    _processedEvents[eventId] = DateTime.now();
  }

  /// Cleanup events older than retention duration to prevent memory leaks
  static void _cleanupOldEvents() {
    final cutoff = DateTime.now().subtract(_eventRetentionDuration);
    _processedEvents.removeWhere((_, timestamp) => timestamp.isBefore(cutoff));
  }
  ...
  FutureOr<Result> handleCall(Session session, Request request) async {
    try {
      ...
      if (eventId == null || eventType == null || eventObject == null) {
      ...
      session.log('Processing Stripe event: $eventType (ID: $eventId)');

      // Use executor to ensure only one webhook processes at a time.
      // This prevents race conditions between concurrent webhook deliveries.
      await _webhookExecutor.scheduleTask(() async {
        ...
        session.log('Event processed successfully: $eventId ($eventType)');
      });
  ...
  Future<void> _handleSubscriptionCreated(
    ...
    await session.db.transaction((transaction) async {
      ...
      // NOTE: We do NOT add credits here because checkout.session.completed
      // already handles credit addition for new subscriptions. Adding credits
      // in both places would cause duplicate credits.
      // This handler only updates the account info (Stripe IDs, plan tier, etc.)
